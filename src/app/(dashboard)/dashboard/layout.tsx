...
const Layout = async ({ children }: LayoutProps) => {
  ...
  return (
    <div className='w-full flex h-screen'>
      <div className='flex h-full w-full max-w-xs grow flex-col gap-y-5 border-r border-gray-200 bg-white px-6'>
        <Link href='/dashboard' className='flex h-26 shrink-0 items-center'>
          <Icons.Logo className='h-8 w-auto text-indigo-600' />
        </Link>
        {friends && friends.length > 0 ? <div className='text-xs font-semibold leading-6 text-gray-400'>Your chats</div> : null}
                  ...
                  return (
                    <li key={option.id}>
                      ...
                      >
                        ...
                        {/* // cut name by dots don't put it on a new line when shrinked */}
                      </Link>
                    </li>
                  );
                })}
            ...
            <li className='-mx-6 mt-auto flex items-center'>
              <div className='flex flex-1 items-center gap-x-4 px-6 py-3 text-sm font-semibold leading-6 text-gray-900'>
                ...
                <div className='flex flex-col'>
                  ...
                  {/* visually impared - doesn't show them their own name */}
                  <span className='text-xs text-zine-400' aria-hidden='true'>
                    {session.user.email}
                  </span>
                </div>
              </div>
